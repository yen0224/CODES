...
int main(int argc, char const *argv[])
{
    ...
    //86400*365=31536000
    long yearSecond = 31536000;
    //birth rate per year
    double birthPerYear = yearSecond / 7;
    //death rate per year
    double deathPerYear = yearSecond / 13;
    //immigrant rate per year
    double immigrantPerYear = yearSecond / 45;
    //output the current population
    cout << "Current population is " << current_amount << endl;
    //use for loop to output the population for the next 5 years
    for (int i = 1; i < 6; i++)
    {
